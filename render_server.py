...
with open('/mnt/kimi/upload/user_pasted_clipboard_long_content_as_file_from flask import Fl.txt', 'r') as f:
    original_code = f.read()

# Add uuid import after the imports section
import_end = original_code.find("app = Flask(__name__)")
fixed_code = original_code[:import_end] + "import uuid\n\n" + original_code[import_end:]

# Add webhook calls - find the success return and add webhook before it
success_webhook = '''
    # Report status back to Cloudflare Worker via webhook
    ...
    if webhook_url:
        try:
            requests.post(webhook_url, json={
                "job_id": job_id,
                "status": final_status,
                "youtube_id": video_id if not TEST_MODE else None,
                "script": script
            }, timeout=10)
            print(f"[{job_id}] Webhook notification sent to {webhook_url}")
        except Exception as wh_err:
            print(f"[{job_id}] Webhook failed (non-critical): {wh_err}")
...
fixed_code = fixed_code.replace(old_success, success_webhook)

# Add webhook for error case
error_webhook = '''
...
fixed_code = fixed_code.replace(old_error, error_webhook)

# Save to file
with open('/mnt/kimi/output/fixed_render_pipeline.py', 'w') as f:
    f.write(fixed_code)

print("✅ Fixed Render Pipeline saved to: /mnt/kimi/output/fixed_render_pipeline.py")
print("\nKey fixes:")
print("- Added 'import uuid' at the top")
print("- Added webhook call on success (reports back to Cloudflare)")
print("- Added webhook call on failure")
print("- Webhook calls are wrapped in try/except (non-blocking)")
